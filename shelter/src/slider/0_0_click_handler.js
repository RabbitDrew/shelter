...
const clickHandler = function (left, right) {
    if (left && !right ) {
        console.log('left')
        stateSwitcher(stateStorage, true, false)
        getShuffledBaseArr = getShuffledArr(cardsArr)
        newCards = getRenderEl(getShuffledBaseArr , 3)
        createStateArr(newCards, stateStorage, cardsArr, true, false)
        ...
        console.log('right')
        stateSwitcher(stateStorage, false, true)
        getShuffledBaseArr = getShuffledArr(cardsArr)
        newCards = getRenderEl(getShuffledBaseArr , 3)
        createStateArr(newCards, stateStorage, cardsArr, true, false)
        oldCards = getRenderCardsFromStateData(stateStorage, 3)
        setTimeout (() => {
            if (stateStorage.mainStateSlider === true ){
                oldCards.length = 0
                setTimeout (()=> {addPrevSlide(sliderLine, newCards, renderEl)},7)
